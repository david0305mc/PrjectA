...
static public class Localization
{
	...
	static public bool LoadCSV(TextAsset asset, bool merge = false, char separator = '|') { return LoadCSV(asset.bytes, asset, merge, separator); }

	/// <summary>
	/// Load the specified CSV file.
	/// </summary>

	static public bool LoadCSV(byte[] bytes, bool merge = false, char separator = '|') { return LoadCSV(bytes, null, merge, separator); }
	...
	static bool LoadCSV(byte[] bytes, TextAsset asset, bool merge = false, char separator = '|')
	{
		if (bytes == null) return false;
		ByteReader reader = new ByteReader(bytes);

		// The first line should contain "KEY", followed by languages.
		BetterList<string> header = reader.ReadCSV(separator);
		...
		for (; ; )
		{
			BetterList<string> temp = reader.ReadCSV(separator);
	...
	static public string Get(string key, string specificLanguage = null)
	{
		...
		if (lanIdx != -1 && mDictionary.TryGetValue(key, out vals))
		{
			if (lanIdx < vals.Length)
			{
				string s = vals[lanIdx];
